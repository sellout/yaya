...
There is no cycle at the target level, but there is one at the package level.

Running ~stack test~ on  this project with GHC 8.0.2 succeeds, but building it with GHC 8.2.1 fails with the following error:
